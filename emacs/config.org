...
#+begin_src elisp
(use-package hexl
  :ensure nil ; hexl-mode is built-in
  :hook ((find-file . my/maybe-open-in-hexl-mode))
  :config
  (defun my/binary-file-p (file)
    "Return non-nil if FILE is binary."
    (let ((coding-system-for-read 'no-conversion))
      (with-temp-buffer
        (insert-file-contents-literally file nil 0 1024)
        (goto-char (point-min))
        (re-search-forward "[\x00-\x08\x0E-\x1F]" nil t))))

  (defun my/maybe-open-in-hexl-mode ()
    "Ask to open binary files in `hexl-mode`."
    (when (and buffer-file-name
               (my/binary-file-p buffer-file-name)
               (not (eq major-mode 'hexl-mode)))
      (when (y-or-n-p (format "File `%s` looks binary. Open in hexl-mode? " buffer-file-name))
        (hexl-mode)))))
#+end_src
