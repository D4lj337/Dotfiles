...
(straight-use-package 'url-util)
(straight-use-package 'dom)

;;; emacs-search.el --- A tool to search DuckDuckGo and display AI answers.

;;; Commentary:
;; This package provides a simple interface to search DuckDuckGo and display
;; the AI-powered instant answers directly within Emacs.
...
(defvar emacs-search-user-agent "Mozilla/5.0 (X11; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/115.0"
  "The User-Agent string to use for requests.")

(defun emacs-search--extract-ai-answer (html-content)
  "Parse HTML-CONTENT and extract the text from the AI answer block.
This function searches for a specific div with class 'zci__result'
in the provided HTML and returns its trimmed text content."
  (with-temp-buffer
    (insert html-content)
    (goto-char (point-min))
    ;; Use libxml to parse the buffer content into a DOM structure.
    (let* ((dom (libxml-parse-html-region (point-min) (point-max)))
           ;; Find the first element with the class "zci__result".
           (zci-node (car (dom-by-class dom "zci__result"))))
      (if zci-node
          ;; If found, extract and clean up the text content.
          (string-trim (dom-text zci-node))
        ;; Otherwise, return nil.
        nil))))

(defun emacs-search (query)
  "Search DuckDuckGo for QUERY and display the AI-powered answer.

When called interactively, prompts for a search QUERY in the
minibuffer. It then fetches the HTML content from DuckDuckGo,
parses it to find the AI-powered \"zero-click info\" answer,
and displays the result in a new buffer named after the query.

If no AI answer is found, the buffer will display the message
\"no search available\". The result buffer is set to
`fundamental-mode`.

The User-Agent for the request can be customized by setting the
`emacs-search-user-agent` variable."
  (interactive "sSearch DuckDuckGo for: ")
  (let ((url-request-extra-headers `(("User-Agent" . ,emacs-search-user-agent)))
        (search-url (format "https://html.duckduckgo.com/html/?q=%s"
                            (url-hexify-string query))))
    (let ((result-buffer (url-retrieve-synchronously search-url)))
      (if (not result-buffer)
          (message "Failed to fetch search results.")
        (let ((html-content (with-current-buffer result-buffer
                              (buffer-string))))
          (kill-buffer result-buffer) ; Clean up the temp buffer
          (let* ((answer (emacs-search--extract-ai-answer html-content))
                 (display-buffer (get-buffer-create (format "*emacs-search: %s*" query))))
            (with-current-buffer display-buffer
              (erase-buffer)
              (fundamental-mode)
              (insert (if answer answer "no search available"))
              (goto-char (point-min))
              (switch-to-buffer-preserve-window (current-buffer)))))))))
